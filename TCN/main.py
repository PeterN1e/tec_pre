...
def main():
    ...
    tec_train = TrainModel(model = model,train_loader=train_dataloader,test_loader=test_dataloader,criterion=criterion_mse,optimizer=optimizer,scheduler=scheduler)
    train_losses, test_losses = tec_train(epochs_num)

    torch.save(model.state_dict(), "tcn_model\\model_state_dict.pth")
...
def model_predict_only():
    ...
    model.load_state_dict(torch.load("tcn_model\\model_state_dict.pth", map_location=device,weights_only=True))
